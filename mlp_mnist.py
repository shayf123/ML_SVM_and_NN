...
class KerasMnist(object):
    ...
    def build_model_no_skip(self):
        ...
        dim_lastlayer = self.num_classes
        net_model = Sequential()
        net_model.add(Dense(self.hidden_layer_dims[0], activation='relu', input_dim=self.input_dim))
        for dim_layer in self.hidden_layer_dims[1:]:
            net_model.add(Dense(dim_layer, activation='relu'))
        net_model.add(Dense(dim_lastlayer, activation='softmax'))
